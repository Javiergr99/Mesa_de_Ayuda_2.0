...
export function OrganizerPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Button variant="secondary" size="icon"><ChevronLeft className="h-4 w-4" /></Button><h2 className="min-w-48 text-center text-base font-bold text-slate-900">Octubre 2026</h2><Button variant="secondary" size="icon"><ChevronRight className="h-4 w-4" /></Button></div><div className="flex rounded-lg bg-slate-100 p-1">{[{value:"month",label:"Mensual"},{value:"week",label:"Semanal"},{value:"list",label:"Lista"}].map((item) => <button key={item.value} onClick={() => setView(item.value as typeof view)} className={cn("focus-ring h-9 rounded-md px-4 text-xs font-semibold text-slate-500", view===item.value && "bg-blue-600 text-white shadow-sm")}>{item.label}</button>)}</div></div></Card>
...
function ListView() {
  return <Card className="p-5"><h2 className="font-bold text-slate-900">Próximos eventos</h2><div className="mt-4 space-y-3">{events.map((event)=><motion.article key={event.id} whileHover={{x:3}} className="flex items-center gap-4 rounded-xl border border-slate-200 p-4"><span className="grid h-11 w-11 place-items-center rounded-lg bg-blue-50 text-blue-600"><Clock3 className="h-5 w-5"/></span><div className="flex-1"><p className="text-sm font-bold text-slate-900">{event.title}</p><p className="mt-1 text-xs text-slate-500">Octubre {event.day}, 2026 · {event.time}</p></div><Button variant="secondary" size="icon"><List className="h-4 w-4"/></Button></motion.article>)}</div></Card>;
}
