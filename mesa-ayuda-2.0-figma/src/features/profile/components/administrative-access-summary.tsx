      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-start gap-3 rounded-xl border border-violet-100 bg-violet-50/80 px-3.5 py-3 text-violet-900">
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold text-violet-700">{role}</p>
        ...
        <ul className="profile-permission-grid mt-4 grid overflow-hidden rounded-xl border border-[var(--ui-border)] sm:grid-cols-2 lg:grid-cols-3">
          {permissions.map((permission) => (
            <li
              key={permission}
              className="profile-permission-item flex min-w-0 items-center gap-2.5 px-3.5 py-3 text-xs font-semibold text-[var(--ui-text-primary)]"
            >
              <CheckCircle2
