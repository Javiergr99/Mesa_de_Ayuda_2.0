  ...
  return (
    <article className="flex min-h-[154px] flex-col rounded-xl border border-[var(--ui-border)] bg-[var(--ui-surface)] px-5 py-4 shadow-[var(--ui-card-shadow)]">
      <div className="flex items-start justify-between gap-4">
        ...
        {helper ? (
          <span className={cn("pt-1 text-[11px] font-bold", classes.accent)}>
            {helper}
          </span>
        ) : null}
      </div>
      ...
      <p className="mt-1.5 text-[12px] font-semibold text-[var(--ui-text-secondary)]">
        {title}
      </p>

      <div className="mt-auto border-t border-slate-200/80 pt-3">
        <p className="text-[11px] leading-4 text-[var(--ui-text-secondary)]">
          {detail}
        </p>
      </div>
    </article>
