  ...
  return (
    ...
    >
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <ReadonlyValue
          label="Nombre(s)"
          value={raw.nombre ?? attention.requester}
        />
        <ReadonlyValue
          label="Primer apellido"
          value={raw.primer_apellido ?? "—"}
        />
        <ReadonlyValue
          label="Segundo apellido"
          value={raw.segundo_apellido ?? "—"}
        />
        <ReadonlyValue
...
export function AttentionContextSection({
  attention,
}: {
  attention: Attention;
}) {
  return (
    <SectionCard
      title="Ubicación e institución"
      icon={<MapPin className="h-4 w-4" />}
    >
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <ReadonlyValue
          label="Estado / PFPNNA"
          value={attention.entity}
        />
        <ReadonlyValue
          label="Institución, área o instancia"
          value={attention.instance}
        />
        <ReadonlyValue
          label="Fecha de atención"
          value={formatDate(attention.date)}
        />
        <ReadonlyValue
          label="Hora de atención"
          value={formatTime(attention.time)}
        />
      </div>
    </SectionCard>
  );
...
export function AttentionRequestSection({
  attention,
}: {
  attention: Attention;
}) {
  ...
  return (
    <>
      ...
      <div className="flex items-start gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">
        ...
        <p>
          Este registro se muestra en modo de consulta. Para actualizar su
          estatus o agregar información, ingrese al módulo{" "}
          <Link
            to="/app/seguimiento"
    ...
    <div className={className}>
      ...
      <div
        className={[
          ...
          multiline
            ? "min-h-20 whitespace-pre-wrap leading-6"
            : "min-h-10",
        ].join(" ")}
      >
        {value || "—"}
